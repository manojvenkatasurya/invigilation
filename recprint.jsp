...
stmt1=(Statement)session.getAttribute("stmt1");
stmt2=(Statement)session.getAttribute("stmt2");
stmt3=(Statement)session.getAttribute("stmt3");
rs=stmt1.executeQuery("select * from records where e_name='"+e_name+"'");
int type=0;
			if(rs.next())
				type=rs.getInt("type");
			String ex;
	...
	<table border=1 style="border-collapse:collapse">
		<tr>
			...
			<td id="tdcenter">Name of the faculty</td>
				<jsp:scriptlet>
				ResultSet dates=stmt2.executeQuery("select distinct date from records where e_name='"+e_name+"'");
				while(dates.next())
				{
		...
		<jsp:scriptlet>
		ResultSet rse=stmt3.executeQuery("select distinct emp_id from records where e_name='"+e_name+"'");
		while(rse.next())
		{
			String id=rse.getString("emp_id");
			ResultSet rs=stmt1.executeQuery("select * from teacher where emp_id='"+id+"'");
			rs.next();
		...
		dates.previous();
								...
								{
										String date=dates.getString("date");
										rs=stmt1.executeQuery("select * from records where date='"+date+"' and emp_id='"+id+"' and e_name='"+e_name+"'");
										int p=0;
										if(rs.next())
											p=1;
										rs.close();
